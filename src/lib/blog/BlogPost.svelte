...
<svelte:head>
	...
	<link
		...
		href="https://stevenwaterman.uk/rss.xml"
		title="Steven Waterman's Blog"
	/>
					...
					<div class="topSection">
						<div class="left">
							...
							{#if post.original !== undefined}
								<p class="original">
									Originally posted on <a href={post.original.link}>{post.original.text}</a>
								</p>
							{/if}
						...
						<!-- {#if post.original === undefined}
							<applause-button
								url={`https://stevenwaterman.uk/${id}`}
								color="var(--blue-1)"
								multiclap="true"
							/>
						{:else}
